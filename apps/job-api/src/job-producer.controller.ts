...
import { JobPayload } from './job-type/job.payload/job.payload';
...
export class JobProducerController {
  ...
  @Post('job')
  async createJobRequest<T extends JobPayload>(
    @Body() createJobRequest: CreateJobRequest<T>,
  ): Promise<void> {
    // factory -> jobRequestPayloadFactory.get.
    const jobGroupParams = this.mapper.map(
      createJobRequest,
      CreateJobRequest,
      // JobGroupParams,
      // factory.get(createJobRequest.jobType),
    );
    console.log('jobGroupParams', jobGroupParams);
